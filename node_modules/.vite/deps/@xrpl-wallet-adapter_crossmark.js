import {
  XRPLWalletAccount
} from "./chunk-2XLKPT7H.js";
import {
  XRPL_MAINNET
} from "./chunk-EY6PB5PI.js";
import "./chunk-JEPBNUQE.js";
import {
  __privateAdd,
  __privateGet,
  __privateMethod,
  __privateSet
} from "./chunk-SNAQBZPT.js";

// node_modules/@xrpl-wallet-adapter/crossmark/dist/index.mjs
var _name, _icon, _accounts, _listeners, _connect, _signTransaction, _signAndSubmitTransaction, _on, _CrossmarkWallet_instances, emit_fn, off_fn;
var _CrossmarkWallet = class _CrossmarkWallet {
  constructor() {
    __privateAdd(this, _CrossmarkWallet_instances);
    __privateAdd(this, _name, "Crossmark");
    __privateAdd(this, _icon, "data:image/png;base64,");
    __privateAdd(this, _accounts, []);
    __privateAdd(this, _listeners, {});
    __privateAdd(this, _connect, async ({ silent = false } = {}) => {
      let address;
      if (silent) {
        address = window.xrpl.crossmark.session.address;
      } else {
        const { response } = await window.xrpl.crossmark.async.signInAndWait();
        address = response.data.address;
      }
      if (!address) return { accounts: [] };
      __privateSet(this, _accounts, [new XRPLWalletAccount(address)]);
      __privateMethod(this, _CrossmarkWallet_instances, emit_fn).call(this, "change", { accounts: this.accounts });
      return {
        accounts: this.accounts
      };
    });
    __privateAdd(this, _signTransaction, async ({ tx_json, account, network }) => {
      const { response } = await window.xrpl.crossmark.async.signAndWait(tx_json);
      return {
        signed_tx_blob: response.data.txBlob
      };
    });
    __privateAdd(this, _signAndSubmitTransaction, async ({ tx_json, account, network }) => {
      const { response } = await window.xrpl.crossmark.async.signAndSubmitAndWait(tx_json);
      return {
        tx_json: response.data.resp.result,
        tx_hash: response.data.resp.result.hash
      };
    });
    __privateAdd(this, _on, (event, listener) => {
      var _a;
      if (__privateGet(this, _listeners)[event]) (_a = __privateGet(this, _listeners)[event]) == null ? void 0 : _a.push(listener);
      else __privateGet(this, _listeners)[event] = [listener];
      return () => __privateMethod(this, _CrossmarkWallet_instances, off_fn).call(this, event, listener);
    });
    if (new.target === _CrossmarkWallet) {
      Object.freeze(this);
    }
  }
  get version() {
    return "1.0.0";
  }
  get name() {
    return __privateGet(this, _name);
  }
  get icon() {
    return __privateGet(this, _icon);
  }
  get chains() {
    return [XRPL_MAINNET];
  }
  get features() {
    return {
      "standard:connect": {
        version: "1.0.0",
        connect: __privateGet(this, _connect)
      },
      "standard:events": {
        version: "1.0.0",
        on: __privateGet(this, _on)
      },
      "xrpl:signTransaction": {
        version: "1.0.0",
        signTransaction: __privateGet(this, _signTransaction)
      },
      "xrpl:signAndSubmitTransaction": {
        version: "1.0.0",
        signAndSubmitTransaction: __privateGet(this, _signAndSubmitTransaction)
      }
    };
  }
  get accounts() {
    return __privateGet(this, _accounts);
  }
};
_name = new WeakMap();
_icon = new WeakMap();
_accounts = new WeakMap();
_listeners = new WeakMap();
_connect = new WeakMap();
_signTransaction = new WeakMap();
_signAndSubmitTransaction = new WeakMap();
_on = new WeakMap();
_CrossmarkWallet_instances = new WeakSet();
emit_fn = function(event, ...args) {
  var _a;
  (_a = __privateGet(this, _listeners)[event]) == null ? void 0 : _a.forEach((listener) => listener.apply(null, args));
};
off_fn = function(event, listener) {
  var _a;
  __privateGet(this, _listeners)[event] = (_a = __privateGet(this, _listeners)[event]) == null ? void 0 : _a.filter((existingListener) => listener !== existingListener);
};
var CrossmarkWallet = _CrossmarkWallet;
export {
  CrossmarkWallet
};
//# sourceMappingURL=@xrpl-wallet-adapter_crossmark.js.map
