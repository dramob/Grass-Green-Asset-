...
class LocalWallet_TESTONLY {
  ...
  get chains() {
    return [core.XRPL_TESTNET, core.XRPL_DEVNET, core.XAHAU_TESTNET];
  }
  get features() {
    return {
      "standard:connect": {
  ...
  constructor({ additionalNetworks = {}, autoFaucetNetwork = "xrpl:1" } = {}) {
  ...
  #connect = async ({ silent = false } = {}) => {
    ...
    if (!silent && !secret) {
      const seed = xrpl.Wallet.generate().seed;
    ...
    const response = await client.request({
      ...
      if (e.message === "Account not found.") return void 0;
      throw e;
    });
    ...
    this.#accounts = [new base.XRPLWalletAccount(account.address)];
    this.#emit("change", { accounts: this.accounts });
    return {
      accounts: this.accounts
    };
  };
  #signTransaction = async ({ tx_json, account, network }) => {
    ...
    const definitions = new rippleBinaryCodec.XrplDefinitions(JSON.parse(JSON.stringify(rawDefinitions)));
    const prepared = await this.#autofill(client, tx_json, acc, definitions);
    await client.disconnect();
    const tx_blob = rippleBinaryCodec.encodeForSigning(prepared, definitions);
    const signedTransaction = rippleKeypairs.sign(tx_blob, acc.privateKey);
    return {
      signed_tx_blob: signedTransaction
    };
  };
  #signAndSubmitTransaction = async ({ tx_json, account, network }) => {
    const acc = this.#getAccount();
    if (!acc) throw new Error("Wallet not connected");
    const server = this.#getWssEndpointFromNetwork(network);
    const client = new xrpl.Client(server);
    ...
    const signature = rippleKeypairs.sign(rippleBinaryCodec.encodeForSigning(prepared, definitions), acc.privateKey);
    prepared.TxnSignature = signature;
    const tx_blob = rippleBinaryCodec.encode(prepared, definitions);
    const response = await client.request({ command: "submit", tx_blob });
    const txResponse = await sugar.waitForFinalTransactionOutcome(
      client,
  ...
  #autofill = async (client, transaction, account, definitions) => {
    ...
    if (!transaction.NetworkID && networkID !== void 0 && networkID > 1024) transaction.NetworkID = networkID;
    if (transaction.Fee === void 0) {
      const {
      ...
      const hasHookAmendment = !!amendments?.Amendments && amendments.Amendments.indexOf("ECE6819DBA5DB528F1A241695F5A9811EF99467CDE22510954FD357780BBD078") > -1;
      const tx_blob = hasHookAmendment ? rippleBinaryCodec.encode({ ...transaction, SigningPubKey: "", Fee: "0" }, definitions) : void 0;
      const response = await client.request({
        command: "fee",
        tx_blob
      });
...
exports.LocalWallet_TESTONLY = LocalWallet_TESTONLY;
