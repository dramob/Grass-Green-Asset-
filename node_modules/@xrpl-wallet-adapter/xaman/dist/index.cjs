...
var xummOauth2Pkce = require('xumm-oauth2-pkce');
...
class XamanWallet {
  ...
  get chains() {
    return [core.XRPL_MAINNET, core.XRPL_TESTNET, core.XRPL_DEVNET, core.XAHAU_MAINNET, core.XAHAU_TESTNET];
  }
  get features() {
    return {
      "standard:connect": {
  ...
  constructor(apiKey) {
    ...
    this.#client = new xummOauth2Pkce.XummPkce(apiKey);
  }
  #connect = async ({ silent = false } = {}) => {
    ...
    this.#accounts = [new base.XRPLWalletAccount(resolvedState.me.account)];
  ...
  #signAndSubmitTransaction = async ({ tx_json, account, network }) => {
    ...
    const client = new xrpl.Client(payload.response.dispatched_to);
  ...
  #getXamanNetworkNameFromChain(chain) {
    switch (chain) {
